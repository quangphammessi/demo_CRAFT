...
def test(file_path):
    ...
    if args.cuda:
        net.load_state_dict(copyStateDict(torch.load(os.path.join(os.path.dirname(__file__), args.trained_model))))
    else:
        net.load_state_dict(copyStateDict(torch.load(os.path.join(os.path.dirname(__file__), args.trained_model), map_location='cpu')))
    ...
    # Switch net sang eval va bo gradient (ko backward de tiet kiem GPU)
    net.eval()
